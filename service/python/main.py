...
from utils.limiter import rate_limit_by_ip
from utils.message_bodys import *
from database_option import models, database, crud, schemas
from utils.util import get_key_usage, get_proxies, num_tokens_from_messages
...
MAX_REQUEST_PER_HOUR = int(os.getenv("MAX_REQUEST_PER_HOUR", "0"))


@router.post('/chat-process')
@rate_limit_by_ip(MAX_REQUEST_PER_HOUR)
async def chat_process(data: ChatProcessRequest, db: Session = Depends(crud.get_db)):
	...
	# 创建会话
	chat_reply_process = await asyncio.get_running_loop().run_in_executor(None,
	...
	async def generate():
		for index, chat in enumerate(chat_reply_process):
			...
			chat_response.answer_token = await num_tokens_from_messages([{"role": "system", "content": chat_response.text}],
																																	model=model)
...
@router.post("/config", response_model=ConfigResponse, summary="配置文件记录")
async def config():
...
@router.post("/session", response_model=SessionResponse, summary="")
async def session():
...
@router.post("/verify", response_model=VerifyResponse, summary="验证授权")
...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(description='Starts the application')
	parser.add_argument('--port', type=int, default=3002, help='the port to listen on')
	args = parser.parse_args()

	port = args.port
	uvicorn.run(app=f"{Path(__file__).stem}:app", host="0.0.0.0", port=port, reload=True)
